//! Fonts, colour and the metrics the form is laid out on.
//!
//! Two things here are accessibility features rather than decoration. The first
//! is the bundled bold face: at the sizes this app uses, a light weight is the
//! single biggest readability cost, and egui ships only Ubuntu-Light. The second
//! is the palette, which is written down as explicit pairs so the contrast
//! ratios can be reasoned about instead of inherited — every text colour below
//! reaches at least 4.5:1 against the surface it is drawn on, and the ones that
//! carry meaning on their own (status, focus) reach 7:1 where the design allows.
//!
...
/// install as it does on a Mac. Shared with the `gcm` console, which is where
/// the file and its licence came from.
const UBUNTU_BOLD: &[u8] = include_bytes!("../assets/fonts/Ubuntu-Bold.ttf");

/// Every control in the form is this wide, so the eye — and a screen magnifier —
/// only has to track one left edge and one right edge down the whole window.
pub const FORM_WIDTH: f32 = 460.0;
...
pub fn apply(ctx: &egui::Context) {
    ...
    ctx.all_styles_mut(|style| {
        // Generous by egui's standards. Everything on screen is a form control
        // the user has to hit, read and tell apart from its neighbour.
        style.spacing.item_spacing = egui::vec2(10.0, 10.0);
        style.spacing.button_padding = egui::vec2(12.0, 8.0);
        style.spacing.interact_size.y = CONTROL_HEIGHT;
        style.spacing.combo_width = FORM_WIDTH;
