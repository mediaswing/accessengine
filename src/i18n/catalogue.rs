...
//! pane.read.hint = "Choisissez un fichier, une voix et ce qu'il faut en faire"
//!
//! # A long one, continued by putting another quoted string underneath.
//! settings.sounds.hint = "Un son bref au démarrage, un carillon en cas de "
//!                        "réussite, et un ton plus grave en cas de problème."
//! ```
//!
...
mod tests {
    ...
    fn a_quoted_line_underneath_continues_the_entry_above() {
        let cat = Catalogue::parse(
            r#"
                settings.hint = "A short tone as the work begins, "
                                "and a chime for success."
            "#,
        );
        assert_eq!(
            cat.get("settings.hint"),
            Some("A short tone as the work begins, and a chime for success.")
        );
        assert!(cat.problems.is_empty(), "{:?}", cat.problems);
    }
    ...
    fn placeholders_are_substituted_by_name() {
        assert_eq!(
            fill(
                "Saved {name} as {format}.",
                &[("name", "a.txt"), ("format", "MP3")]
            ),
            "Saved a.txt as MP3."
        );
    }
    ...
    fn a_translation_may_reorder_the_values() {
        let values = [("name", "a.txt"), ("format", "MP3")];
        assert_eq!(
            fill("{format} enregistré sous {name}.", &values),
            "MP3 enregistré sous a.txt."
        );
    }
